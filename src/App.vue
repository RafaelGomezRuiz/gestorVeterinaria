<script setup>
import { ref, reactive, onMounted, watch } from 'vue';
import { uid } from 'uid';

import Formulario from './components/Formulario.vue';
import Header from './components/Header.vue';
import Paciente from './components/Paciente.vue';

const pacientes = ref([]);
const paciente = reactive({
    id:null,
    nombre: "",
    propietario: "",
    correo: "",
    alta: "",
    sintomas: ""
});

const setLocalStorage = ()=>{
    localStorage.setItem('pacientes', JSON.stringify(pacientes.value));
}

const getLocalStorage = ()=>{
    const valor =  localStorage.getItem('pacientes');
    console.log("valores que llegan ", valor);

    if (valor) {
        pacientes.value = JSON.parse(valor);
    } 
}

const guardar = ()=>{
    if(paciente.id){
        const {id} = paciente;
        
        const i = pacientes.value.indexOf(id);
        pacientes.value[i] = {...paciente};
    }else{
        console.log('asre ', paciente);
        pacientes.value.push({...paciente, id: uid()});
        setLocalStorage(pacientes);
    }
    paciente.nombre="";
    paciente.propietario= "",
    paciente.correo= "",
    paciente.alta= "",
    paciente.sintomas= ""
    paciente.id= null
}



const actualizarPaciente = (id)=>{
    const pacienteEdit = pacientes.value.filter((paciente)=> paciente.id === id)[0]
    Object.assign(paciente, pacienteEdit);
    console.log("actualizar paciente", paciente);
}

const eliminarPaciente = (id)=>{
    console.log("id ", id);
    pacientes.value = pacientes.value.filter((paciente)=> paciente.id === id)[0]
    // Object.assign(paciente, pacienteEdit);
}

watch(pacientes, ()=>{
    setLocalStorage(pacientes.value)
}, {
    deep:true
})

onMounted(()=>{
    getLocalStorage();
})
</script>

<template>
    <div class="container mx-auto mt-20">
        <Header />
        <div class="md:flex mt-12">
            <Formulario
                v-model:nombre="paciente.nombre"
                v-model:propietario="paciente.propietario"
                v-model:correo="paciente.correo"
                v-model:alta="paciente.alta"
                v-model:sintomas="paciente.sintomas"
                @guardar-paciente="guardar"
                :id="paciente.id"
            />
            <div class="md:w-1/2 md:h-screen overflow-y-scroll">
                <h3 class="font-black text-3xl text-center">
                    Administra tus pacientes
                </h3>
                <div v-if="pacientes.length > 0">
                    <p class="text-lg mt-5 text-center mb-10">
                        Información de 
                        <span class="text-indigo-600">Pacientes</span>
                    </p>

                    <Paciente v-for="paciente in pacientes"
                        :paciente="paciente"
                        @actualizar-paciente="actualizarPaciente"
                        @eliminar-paciente="eliminarPaciente"
                    />
                </div>
                <p v-else class="mt-10 text-2xl text-center"> No Hay Pacientes</p>
            </div>
        </div>
    </div>
</template>

 
